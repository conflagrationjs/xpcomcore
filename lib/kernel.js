...
function Kernel(mixinScope) {
  ...
  if (mixinScope) {
    ...
    for (var p in myPrototype) { mixinScope.__defineGetter__(p, makeGetter(p)); };
  };
  return myPrototype;
};
...
Kernel.prototype = {
  ...
  CURRENT_FILE: function() {
    ...
    // If we're loaded as a resource, resolve ourselves to an actual filesystem path and return that.
    ...
    // If we're loaded as a file, just return our path.
    } else if (thisFileURI.scheme == "file") {
      return thisFileURI.QueryInterface(Ci.nsIURL).path;
    } else {
      throw(new SelfConceptError("The filesystem location of the current file could not be determined."));
    }
  },
